...
def slice_and_export_to_anki(video_path, srt_en, srt_pt, deck_name, work_dir, field_mapping=None):
    ...
    def process_media(args):
        ...
        cmd = [
            ...
            # --- Output 1: WebM (video + audio) ---
            "-map", "0:v:0",
            "-map", "0:a:0",
            "-c:v", "libvpx-vp9",
            "-cpu-used", "5",                   # speed mode for VP9
            "-crf", "30",                       # acceptable quality
            "-b:v", "0",                        # required for VP9 CRF
            "-c:a", "libopus",
            "-b:a", "64k",
    ...
    def upload_card(card_data):
        ...
        # Clip: vid_...webm — only the WebM video filename (without [sound:] tag)
        ...
        # Index: [sound:vid_...webm] — WebM video file with [sound:] tag
